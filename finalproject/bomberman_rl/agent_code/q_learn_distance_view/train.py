...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    # p.state_to_features is defined in callbacks.py
    
    transition = Transition(p.state_to_features(old_game_state), self_action, p.state_to_features(new_game_state), reward_from_events(self, events))
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    #self.transitions.append(Transition(p.state_to_features(last_game_state), last_action, None, reward_from_events(self, events)))
    ...
    transition = Transition(p.state_to_features(last_game_state), last_action, None, reward_from_events(self, events))
    self.transitions.append(transition)
    self.total_step += 1
    if self.total_step % TRANSITION_HISTORY_SIZE == 0:
        for trans in self.transitions:
...
def reward_from_events(self, events: List[str]) -> int:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    game_rewards = {
        ...
        e.KILLED_SELF: -100,
        e.COIN_COLLECTED: 10,
        REDUCED_DISTANCE_TO_NEXT_COIN: 1,
...
def update_Q(self, Transition):
    ...
    self.Q[Transition[0]][action_index] = (1-self.alpha)*self.Q[Transition[0]][action_index] + self.alpha*(Transition[3] + self.gamma*np.max(self.Q[Transition[2]]))
